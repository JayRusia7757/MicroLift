...
Core Workflow
Registration: Users register as Beneficiary, Donor, or Admin via the Auth Service.
Campaign Creation: Beneficiaries create campaigns (uploading thumbnails/docs) via the Campaign Service.
Verification: Admins review campaigns (approve/reject) via the Admin Panel.
Donation: Donors browse active campaigns and donate via the Donation Service (integrated with Mock/Razorpay).
Discovery: All services register with Eureka Server for dynamic discovery.
Routing: The API Gateway routes all frontend requests to the appropriate service.
...
Start Databases: Ensure MySQL is running on port 3306.
Start Backend: Run ./run_microservices.ps1 (Starts all 5 services).
Start Frontend: Run npm run dev in the project root.
Access App: Open http://localhost:5173.
